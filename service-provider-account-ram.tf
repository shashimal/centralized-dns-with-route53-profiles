...
#####################################################
resource "aws_ram_resource_share" "service_provider_tgw_share" {
  name                      = "central-tgw-share"
  allow_external_principals = true
  tags = {
    Name = "service-provider-tgw-share"
  }
}
...
resource "aws_ram_resource_association" "tgw_resource_association" {
  ...
  resource_arn       = aws_ec2_transit_gateway.service_provider_tgw.arn
}
